...
async function createListing(client, newListing){
  const result = await client.db("vehicle-registry").collection("employee-registry").insertOne(newListing);
...
async function createTrafficData(client,classType,trafficData){
  const database = await client.db("vehicle-registry")
  const result ;
  if(classType == "STUDENT") result = await database.collection("student-registry").insertOne(trafficData);
  if(classType == "EMPLOYEE") result = await database.collection("employee-registry").insertOne(trafficData);
  return result;
}
  wss.on("connection", function connection(ws) {
    ws.on("message", function incoming(data) {
      ...
      if (event == "sendData") {
        ...
        try{
          await client.connect()
         const result =  await trafficData(client,classType,{
